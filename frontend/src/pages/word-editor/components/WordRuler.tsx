...
export const WordRuler: React.FC<WordRulerProps> = ({
  ...
  return (
    <div className="w-full flex justify-center py-1 select-none pointer-events-none">
      <div 
        className="relative h-5 bg-[#0f1423] border border-white/10 rounded text-[9px] font-mono text-slate-400 flex items-end shadow-inner"
      ...
      >
        ...
        <div 
          className="absolute top-0 bottom-0 right-0 bg-blue-500/10 border-l border-blue-400/40"
          ...
          {Array.from({ length: totalCm + 1 }).map((_, i) => {
            ...
            return (
              <div key={i} className="flex flex-col items-center">
                <span className={`text-[8px] leading-none ${isZero ? 'text-blue-400 font-bold' : 'text-slate-500'}`}>
                  {i}
                </span>
                <div className={`w-[1px] ${i % 5 === 0 ? 'h-2 bg-slate-400' : 'h-1 bg-slate-600'}`} />
              </div>
            );
          })}
        </div>
