...
import { TrendingUp, TrendingDown, Minus, ShieldCheck, Zap, Users } from 'lucide-react';
...
export const KPICards: React.FC<KPICardsProps> = ({ stats, engine, hasSelectedStudent }) => {
  return (
    <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 ${hasSelectedStudent ? 'animate-cascade-2' : 'animate-cascade-1'}`}>
      ...
      <div className="kpi-card-blue p-5 flex flex-col justify-between shadow-2xl transition-all duration-300 min-h-[105px]">
        <div>
          ...
          <div className="flex items-baseline justify-between gap-2">
            ...
            <span className={`text-xs font-mono font-black flex items-center gap-1 px-2.5 py-0.5 rounded-full ${stats.overallDiff.startsWith('+') ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30' :
              stats.overallDiff.startsWith('-') ? 'bg-rose-500/20 text-rose-300 border border-rose-500/30' :
                'bg-slate-500/20 text-slate-300 border border-slate-500/30'
              }`}>
              {stats.overallDiff.startsWith('+') ? <TrendingUp size={12} /> : stats.overallDiff.startsWith('-') ? <TrendingDown size={12} /> : <Minus size={12} />}
              <span>{stats.overallDiff}</span>
            </span>
          </div>
        </div>
      ...
      <div className="kpi-card-purple p-5 flex flex-col justify-between shadow-2xl transition-all duration-300 min-h-[105px]">
        <div>
          ...
          <div className="flex items-baseline justify-between gap-2">
            ...
            <span className={`text-xs font-bold px-2.5 py-0.5 rounded-full ${engine.consistency_label?.includes('Rất ổn định') ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30' :
        ...
        <div className="pt-3 border-t border-white/5 flex items-center justify-between text-[11px] text-slate-400">
          <span className="flex items-center gap-1.5"><ShieldCheck size={13} className="text-purple-400" /> Chỉ số tin cậy</span>
          <span className="font-mono font-bold text-purple-300">{engine.consistency_score ?? 90}%</span>
        </div>
      </div>

      {/* 3. ATTENDANCE RATE CARD */}
      <div className="kpi-card-green p-5 flex flex-col justify-between shadow-2xl transition-all duration-300 min-h-[105px]">
        <div>
          ...
          <div className="flex items-baseline justify-between gap-2">
            ...
            <span className="text-xs font-mono font-bold bg-[#062419] text-emerald-400 border border-emerald-500/30 px-2.5 py-0.5 rounded-full">
              {stats.sessionCount} Buổi
            </span>
          </div>
        </div>
        <div className="pt-3 border-t border-white/5 flex items-center justify-between text-[11px] text-slate-400">
          <span className="flex items-center gap-1.5"><Users size={13} className="text-emerald-400" /> Điểm danh đầy đủ</span>
      ...
      <div className="kpi-card-amber p-5 flex flex-col justify-between shadow-2xl transition-all duration-300 min-h-[105px]">
        <div>
          ...
          <div className="flex items-baseline justify-between gap-2">
            ...
            <span className={`text-xs font-bold px-2.5 py-0.5 rounded-full ${engine.trend_label?.includes('Tăng') || engine.trend_label?.includes('cải thiện') ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30' :
        ...
        <div className="pt-3 border-t border-white/5 flex items-center justify-between text-[11px] text-slate-400">
          <span className="flex items-center gap-1.5"><Zap size={13} className="text-amber-400" /> Chỉ số PI</span>
