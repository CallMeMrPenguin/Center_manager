...
export const StudentGradeHistoryTable: React.FC<StudentGradeHistoryTableProps> = ({
  ...
  const historyColumns = useMemo<ColumnDef<any>[]>(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const skill = r.check_1_skill || (r.check_1_test_type ? (r.check_1_test_type === 'grammar' ? 'grammar' : 'vocab') : 'vocab');
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const skill = r.check_2_skill || (r.check_2_test_type ? (r.check_2_test_type === 'vocab' ? 'vocab' : 'grammar') : 'grammar');
        const style = getSkillStyle(skill);
  ...
  return (
    <div className={`bg-white dark:bg-[#141d38] border border-slate-200 dark:border-[#24335c] rounded-2xl flex flex-col shadow-sm dark:shadow-xl mb-8 transition-colors ${hasSelectedStudent ? 'animate-cascade-5' : 'animate-cascade-4'}`}>
