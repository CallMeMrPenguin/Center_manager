...
export default function StudentsPage() {
  ...
  return (
    <div className="h-full w-full overflow-y-auto p-6 space-y-6 select-none bg-[#0d101d] font-sans scrollbar-thin">
      ...
      {/* SEARCH AND FILTER BAR WITH COLUMN PICKER */}
      <div className="bg-[#14192b] border border-[#28334e] p-4 rounded-2xl flex flex-wrap items-center justify-between gap-4 shadow-lg">
        <div className="flex flex-wrap items-center gap-3 flex-1">
          ...
          <div className="relative flex-1 min-w-[240px]">
            ...
            <VietnameseInput
              ...
              placeholder="Tìm kiếm theo tên, biệt danh, SĐT phụ huynh, trường học..."
              className="w-full bg-[#1c243c] border border-[#344268] rounded-xl pl-10 pr-4 py-2 text-xs text-white placeholder-slate-400 focus:outline-none focus:border-indigo-500 font-semibold"
            />
          </div>
          ...
          <select
            ...
            className="bg-[#1c243c] border border-[#344268] text-white text-xs font-bold rounded-xl px-3.5 py-2 focus:outline-none focus:border-indigo-500 cursor-pointer"
          >
        ...
        <div className="relative" ref={colPickerRef}>
          ...
          {showColPicker && (
            <div className="absolute right-0 mt-2 w-56 bg-[#161c30] border border-[#303d62] rounded-2xl shadow-2xl p-3 z-30 flex flex-col gap-2">
              <span className="text-[10px] font-black uppercase tracking-wider text-slate-400 px-1">Tùy Chọn Cột</span>
              {[
            ...
            <table className="w-full text-left border-collapse text-xs">
              <thead className="bg-[#1c243c] text-slate-300 uppercase text-[10px] font-black tracking-wider border-b border-[#303d62]">
                <tr>
                  ...
                  {visibleCols.info && <th className="py-3.5 px-4">Thông Tin Cá Nhân</th>}
              ...
              <tbody className="divide-y divide-[#222b45] font-medium bg-[#14192b]">
                {students.slice((currentPage - 1) * pageSize, currentPage * pageSize).map((st, idx) => (
                  <tr key={st.id} className="hover:bg-[#1a223a] transition-colors">
                    ...
                    {/* Uniform student name in clean white text */}
                    {visibleCols.name && (
                    ...
                    {visibleCols.info && (
                      <td className="py-3.5 px-4">
                        <div className="text-slate-300">
                          <span>{st.gender || 'Nam'}</span> • <span>{st.grade || 'Lớp 6'}</span>
                          {st.date_of_birth && <div className="text-[10px] text-slate-400 mt-0.5">Ns: {st.date_of_birth}</div>}
                        </div>
                      </td>
                    )}
