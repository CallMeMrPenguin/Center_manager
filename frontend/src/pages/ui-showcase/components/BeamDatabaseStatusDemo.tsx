...
export function BeamDatabaseStatusDemo() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 p-4 bg-white dark:bg-[#0c0f1e] border border-slate-200 dark:border-[#232f54] rounded-2xl shadow-[0_2px_8px_-2px_rgba(0,0,0,0.06),0_1px_3px_-1px_rgba(0,0,0,0.06)]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-sm font-black text-slate-900 dark:text-white">Kết Nối Cơ Sở Dữ Liệu SQLite</span>
              <span className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full border ${
                ...
                <span className={`w-1.5 h-1.5 rounded-full ${
                  dbStatus === 'online' ? 'bg-emerald-500 animate-pulse' : dbStatus === 'checking' ? 'bg-amber-500' : 'bg-rose-500'
                }`} />
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              <span>Độ trễ phản hồi: <strong className="text-slate-900 dark:text-white font-mono">{latency !== null ? `${latency} ms` : '-'}</strong></span>
              <span className="w-1 h-1 rounded-full bg-slate-400 dark:bg-slate-600" />
              <span>Lần kiểm tra cuối: <strong className="text-slate-700 dark:text-slate-300 font-mono">{lastCheck || '-'}</strong></span>
            </div>
          </div>
        </div>
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-indigo-50 dark:bg-indigo-500/20 hover:bg-indigo-100 dark:hover:bg-indigo-500/30 border border-indigo-200 dark:border-indigo-500/40 text-xs font-bold text-indigo-700 dark:text-indigo-300 transition cursor-pointer active:scale-95 shadow-xs"
        >
      ...
      <div className="bg-white dark:bg-[#080b14] border border-slate-200 dark:border-[#1b2444] p-6 rounded-2xl space-y-4 shadow-[0_2px_8px_-2px_rgba(0,0,0,0.06),0_1px_3px_-1px_rgba(0,0,0,0.06)]">
        ...
        <BeamContainer
          ref={containerRef}
          className="mx-auto flex w-full items-center justify-between rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-[#0a0d1a] px-8 py-16 sm:px-14 md:px-20 shadow-inner min-h-[380px]"
        >
          {/* Node 1: User */}
          <div className="flex flex-col items-center gap-3 shrink-0 z-10">
            ...
            <span className="font-extrabold text-xs text-slate-700 dark:text-slate-200 uppercase tracking-wider whitespace-nowrap">
              Người Dùng
            </span>
          </div>
          ...
          <div className="flex flex-col items-center gap-3 shrink-0 z-10">
            ...
            >
              <Bot className="h-10 w-10 text-purple-600 dark:text-purple-300" />
            </BeamNode>
            <span className="font-extrabold text-xs text-indigo-600 dark:text-indigo-300 uppercase tracking-wider whitespace-nowrap">
              AI Predict Core
            </span>
          </div>
          ...
          <div className="flex flex-col gap-14 shrink-0 z-10">
            ...
            <div className="flex flex-col items-center gap-3">
              ...
              >
                <Zap className="h-7 w-7 text-emerald-500 dark:text-emerald-400" />
              </BeamNode>
              <span className="font-extrabold text-xs text-slate-700 dark:text-slate-200 uppercase tracking-wider whitespace-nowrap">
                Kết Quả
              </span>
