...
import { ShieldAlert, Plus, Trash2, UserPlus, X, RefreshCw } from 'lucide-react';
import { GroupData } from './types';
import { showToast } from '../../Toast';

interface ConflictGroupsSectionProps {
  conflictGroups: GroupData[];
  loading: boolean;
...
export const ConflictGroupsSection: React.FC<ConflictGroupsSectionProps> = ({
  conflictGroups,
  loading,
  onOpenAddMember,
  onCreateGroup,
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h3 className="text-base font-black text-slate-900 dark:text-white flex items-center gap-2">
            ...
            <span>Nhóm Xung Đột</span>
          </h3>
          <p className="text-xs text-slate-500 dark:text-slate-400 font-medium mt-0.5">
            Học sinh thuộc nhóm xung đột sẽ không ngồi cạnh nhau và không chuyển bài cho nhau.
          </p>
        </div>
        ...
        <button
          ...
          className="flex items-center gap-1.5 bg-rose-600 hover:bg-rose-500 text-white px-3.5 py-1.5 rounded-xl text-xs font-black transition cursor-pointer shadow-xs border-0"
        >
          <Plus size={14} />
      ...
      {creating && (
        <form
          ...
          className="bg-white dark:bg-[#121626] border border-rose-200 dark:border-rose-500/30 p-4 rounded-2xl shadow-xs space-y-3"
        >
          <div>
            ...
            <input
              ...
              placeholder="Ví dụ: Nhóm Mâu Thuẫn 1, Nhóm A..."
              className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl px-3 py-2 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-rose-500 font-semibold"
            />
          </div>
          ...
          <div className="flex justify-end gap-2 pt-2 border-t border-slate-100 dark:border-white/5">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="px-4 py-1.5 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-xs font-black transition border-0 cursor-pointer shadow-xs disabled:opacity-50"
            >
      ...
      {loading ? (
        <div className="py-8 text-center text-xs text-slate-400 flex items-center justify-center gap-2">
          ...
          <span>Đang tải danh sách nhóm xung đột...</span>
        </div>
            ...
            >
              <div>
                ...
                <div className="flex items-center justify-between gap-2 pb-2.5 border-b border-slate-100 dark:border-white/5">
                  <div className="flex items-center gap-2 min-w-0">
                    <ShieldAlert size={15} className="text-rose-500 dark:text-rose-400 shrink-0" />
              ...
              <div className="pt-2 border-t border-slate-100 dark:border-white/5">
                <button
                  ...
                  className="text-xs font-bold text-rose-600 dark:text-rose-400 hover:text-rose-700 dark:hover:text-rose-300 flex items-center gap-1 cursor-pointer border-0 bg-transparent"
                >
                  <UserPlus size={13} />
