...
def process_grade(files_dir, grade):
    ...
    src1 = os.path.join(files_dir, f"Vocabulary List - Grade {grade} - 1.docx")
    src2 = os.path.join(files_dir, f"Vocabulary List - Grade {grade} - 2.docx")
    dest = os.path.join(files_dir, f"GS {grade} _ Vocabulary.docx")
    
    print(f"--- Processing Grade {grade} ---")
    if not os.path.exists(src1) or not os.path.exists(src2):
        raise FileNotFoundError(f"Source files '{os.path.basename(src1)}' or '{os.path.basename(src2)}' not found in files folder!")
    ...
    for p in doc.paragraphs:
        ...
        if match:
            ...
            if str(grade) in unit_names_dyn and str(unit_num) in unit_names_dyn[str(grade)]:
                topic_name = unit_names_dyn[str(grade)][str(unit_num)]
