...
def format_vocabulary_file(filepath, grade=None):
    """Applies academic vocabulary formatting rules to a single docx file."""
    if not os.path.exists(filepath):
        raise FileNotFoundError(f"File not found: {filepath}")
        
    # Make backup copy first
    dir_name, filename = os.path.split(filepath)
    formatted_filepath = os.path.join(dir_name, f"Formatted_{filename}")
    shutil.copy2(filepath, formatted_filepath)
    
    doc = docx.Document(formatted_filepath)
    ...
    # 1. Find unit titles and format them
    unit_1_p = None
    for p in doc.paragraphs:
        text = p.text.strip()
        ...
        if match:
            ...
            # If grade is provided, try to rename title
            if grade and grade in UNIT_NAMES and unit_num in UNIT_NAMES[grade]:
                topic_name = UNIT_NAMES[grade][unit_num]
                new_title = f"UNIT {unit_num}: {topic_name}"
            else:
                new_title = f"UNIT {unit_num}"
    ...
    # 4. Convert sections to landscape
    ...
    # 7. Update TOC fields
    try:
        update_docx_fields(formatted_filepath)
    except Exception:
        pass
