...
def repeat_table_header(table):
    ...
    if len(table.rows) > 0:
        ...
        # Configure tblHeader (remove w:val="0" if exists to enable it)
...
def add_toc_to_paragraph(p):
    ...
    run._r.append(instrText)
    run._r.append(fldChar2)
    # Placeholder text so it can be updated
...
def apply_vocabulary_formatting(p, is_single_para):
    ...
    if base_word.startswith('[') and base_word.endswith(']'):
        # Square bracket pattern: whole paragraph bold, no italic, brackets removed
        bold = True
        italic = False
        display_word = base_word[1:-1]
    elif base_word.startswith('{') and base_word.endswith('}'):
        # Curly bracket pattern: italic, no bold, brackets removed
        bold = False
        italic = True
        display_word = base_word[1:-1]
    else:
...
def process_file(src_path, dest_path, grade):
    """Process a single docx file according to formatting rules."""
    ...
    for p in doc.paragraphs[:10]:
        ...
        if re.search(r'Vocabulary\s+List\s*-\s*Grade', text, re.IGNORECASE):
            ...
            print(f"  Removing header line: '{text}'")
    ...
    for p in doc.paragraphs:
        ...
        if match:
            ...
            if grade in UNIT_NAMES and unit_num in UNIT_NAMES[grade]:
                ...
                if unit_num == 1:
                    unit_1_p = p
                    # Unit 1 starts in Section 1, page_break_before is handled by section break
    ...
    if len(doc.paragraphs) > 0:
        ...
        # TOC Note Paragraph (Vietnamese text for vocabulary rules explanation)
    ...
    if len(doc.sections) > 1:
        ...
        footer_p.text = "" # Reset footer paragraph text
        footer_p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        
        # Insert page number only (centered, no "Trang " prefix)
            ...
            if r_idx > 0:
                for cell in row.cells:
                    for p in cell.paragraphs:
                        ...
                        p.paragraph_format.line_spacing = 1.0 # Single
            
            if r_idx == 0:
                continue # Skip header styling in apply_vocabulary_formatting
    ...
    # 7. Update Table of Contents and page number strings dynamically using Word
    update_docx_fields(dest_path)
...
def main():
    ...
    docx_files = [f for f in files_in_dir if f.endswith(".docx") and "Formatted" not in f and "temp" not in f]
    
    print(f"Found source DOCX files: {docx_files}")
    
    for filename in docx_files:
        match = re.search(r'Grade\s+(\d+)', filename, re.IGNORECASE)
        if match:
            grade = int(match.group(1))
            dest_filename = filename.replace(".docx", " - Formatted.docx")
            process_file(filename, dest_filename, grade)
        else:
